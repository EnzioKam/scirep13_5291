function [result, glbestP, bestvaluerec, bestposrec] = hmCSO(Xini, Vini, phi, sig, obj, iters, up, lb)
    % Runs the half-modified Competitive Swarm Optimiser algorithm.
    %
    % Inputs:
    ...
    while iterNum <= N
        ...
        for i = 1:ceil(swarmSize/2)
            if losers(i) <= swarmSize/2 % Apply projection + pertubation on only half of the particles
                ...
                positions(:, losers(i)) = min(max(positions(:, losers(i)), lb), up);
            end
        end
